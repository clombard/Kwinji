...
include ('openinviter.php'); $inviter = new OpenInviter();
...
function oks($oks) {
  if (!empty($oks)) {
    ...
    foreach ($oks as $key => $msg) $contents .= "{$msg}<br >";
    $contents .= "</td></tr></table><br >";
    return $contents;
  }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  if ($step == 'get_contacts') {
    ...
    if (count($ers) == 0) {
      ...
      elseif (FALSE === $contacts = $inviter->getMyContacts()) {  $ers['contacts'] = "Unable to get contacts !";}
      else {
        $import_ok = TRUE;
        $step = 'send_invites';
  ...
  elseif ($step == 'send_invites') {
    if (empty($_POST['provider_box'])) {
      ...
      if ($internal) {
        ...
        if ($inviter->showContacts()) {
          foreach ($_POST as $key => $val) if (strpos($key, 'check_') !== FALSE) {   $selected_contacts[$_POST['email_'. $val]] = $_POST['name_'. $val]; }
    ...
    if (count($ers) == 0) {
      ...
      if ($sendMessage === -1) {
        ...
        foreach ($selected_contacts as $email => $name) mail($email, $message_subject, $message_body, $headers);
...
if (!$done) {
  if ($step == 'get_contacts') {
    ...
    foreach ($oi_services as $type => $providers) {
      ...
      foreach ($providers as $provider => $details) $contents .= "<option value='{$provider}'". ($_POST['provider_box'] == $provider ? ' selected' : '') .">{$details['name']}</option>";
      $contents .= "</optgroup>";
    }
  ...
  else $contents .= "<table class='thTable' cellspacing='0' cellpadding='0' style='border:none;'>
...
$contents .= "<center><a href='http://openinviter.com/'><img src='http://openinviter.com/images/banners/banner_blue_1.gif?nr=51841' border='0' alt='Powered by OpenInviter.com' title='Powered by OpenInviter.com'></a></center>";
if (!$done) {
  if ($step == 'send_invites') {
    if ($inviter->showContacts()) {
      ...
      if (count($contacts) == 0) $contents .= "<tr class='thTableOddRow'><td align='center' style='padding:20px;' colspan='". ($plugType == 'email' ? "3" : "2") {
        ."'>You do not have any contacts in your address book.</td></tr>";
      }
      else {
