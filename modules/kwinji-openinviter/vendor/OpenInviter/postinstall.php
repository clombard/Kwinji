...
set_time_limit(0); $rewrite_config = FALSE;
include ('config.php');
...
function check_cache() {
  if (file_exists('postinstall_results.res')) if (time() - filemtime('postinstall_results.res') < 120) {
...
if (!extension_loaded('dom') OR !class_exists('DOMDocument')) {
  display_message("*NOT OK* - OpenInviter will not run correctly on this system.");
  exit;
}
else display_message("*OK*<br>\n");
...
if ($openinviter_settings['transport'] != $transport) {
  $rewrite_config = TRUE;
  $openinviter_settings['transport'] = $transport;
}
...
if ($openinviter_settings['stats'] != $stats) {
  $rewrite_config = TRUE;
  $openinviter_settings['stats'] = $stats;
}
...
if (!is_writable("{$cookie_path}")) {
  ...
  if (strpos($cookie_path, ";") !== FALSE) $cookie_path = substr($cookie_path, strpos($cookie_path, ";") {
    + 1);
  }
...
if ($openinviter_settings['hosted']) {
  ...
  foreach ($plugins as $type => $dummy) foreach ($dummy as $plugin => $details) {
