...
class openinviter {
  ...
  private function checkConfig() {
    ...
    foreach ($this->configStructure as $option => $details) {
      ...
      if ($ok) if ($details['required'] AND empty($this->settings[$option])) {
  ...
  private function statsOpenDB() {
    ...
    if (function_exists('sqlite_open')) if ($this->statsDB = sqlite_open($this->basePath .'/openinviter_stats.sqlite', 0666)) {
      return TRUE;
    }
    return FALSE;
  }
  ...
  private function statsRecordImport($contacts) {
    ...
    if (!$this->statsDB) if (!$this->statsOpenDB()) {
      return FALSE;
    }
  ...
  public function startPlugin($plugin_name, $getPlugins = FALSE) {
    ...
    elseif (file_exists($this->basePath ."/plugins/{$plugin_name}.plg.php")) {
      ...
      if (file_exists($this->basePath ."/conf/{$plugin_name}.conf")) {
        ...
        else $this->plugin->messageDelay = 1;
        if (!empty($maxMessages)) {
          $this->plugin->maxMessages = $maxMessages;
        }
        else $this->plugin->maxMessages = 10;
      }
    }
    else {
      $this->internalError = "Invalid service provider";
  ...
  public function getPlugins($update = FALSE, $required_details = FALSE) {
    ...
    $valid_cache = FALSE;
    $cache_ts    = 0;
    if (!$update) if (file_exists($cache_path)) {
      include ($cache_path);
    ...
    if (!$valid_cache) {
      ...
      if ($update == TRUE OR $this->settings['hosted'] == FALSE) {
        ...
        if (count($array_file) > 0) {
          ...
          else foreach ($plugins as $key => $vals) {
            foreach ($vals as $key2 => $val2) if (!isset($array_file[$key2]))unset($vals[$key2]);
            if (empty($vals))unset($plugins[$key]);
            else $plugins[$key] = $vals;
          }
          foreach ($array_file as $plugin_key => $file) if (filemtime($file) > $cache_ts OR $reWriteAll) {   $modified_files[$plugin_key] = $file; }
          foreach ($modified_files as $plugin_key => $file) if (file_exists($this->basePath ."/conf/{$plugin_key}.conf")) {
    ...
    if ($required_details) {
      if (!$valid_rcache) {
        foreach ($returnPlugins as $types => $plugins) foreach ($plugins as $plugKey => $plugin) if (!empty($plugin['imported_details'])) {
    ...
    $temp = array();
    if (!empty($returnPlugins)) foreach ($returnPlugins as $type => $type_plugins) {
  ...
  private function checkLoginCredentials($user) {
    ...
    if ($this->currentPlugin['allowed_domains'] AND $is_email) {
      ...
      $user_domain = $temp[1];
      $temp        = FALSE;
      foreach ($this->currentPlugin['allowed_domains'] as $domain) if (preg_match($domain, $user_domain)) {
        $temp = TRUE;
        break;
      }
      if (!$temp) {
  ...
  public function getPluginByDomain($user) {
    ...
    foreach ($this->availablePlugins as $plugin => $details) {
      ...
      elseif (isset($details['detected_domains'])) {  $patterns = $details['detected_domains'];}
      foreach ($patterns as $domain_pattern) if (preg_match($domain_pattern, $user_domain)) {   return $plugin; }
    }
    return FALSE;
  }
