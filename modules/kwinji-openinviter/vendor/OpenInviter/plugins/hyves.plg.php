...
class hyves extends openinviter_base {
  ...
  public function login($user, $pass) {
    ...
    $post_elements['auth_username'] = $user;
    $post_elements['auth_password'] = $pass;
    $post_elements['login'] = 'Login';
  ...
  public function getMyContacts() {
    ...
    while ($hasContacts) {
      ...
      $query = "//a[@class='memberlistname'][not(contains(@href,'". $this->service_user ."'))]";
      $data  = $xpath->query($query);
      foreach ($data as $node) {
        $href = $node->getAttribute('href');
        $name = (string)$node->nodeValue;
        if (!empty($name)) {
          $name = $name[0] . $this->getElementString($name, $name[0], '(');
        }
        if (!empty($href)) {
          $contacts[$href] = !empty($name) ? utf8_decode($name) : FALSE;
          $hasContacts = TRUE;
        }
      }
      $page    = $page + 10;
