...
class hushmail extends openinviter_base {
  ...
  public function login($user, $pass) {
    ...
    $post_elements['user'] = $user;
    $post_elements['passphrase'] = $pass;
  ...
  public function getMyContacts() {
    ...
    $form_action = "https://m.hush.com". $this->getElementString($res, 'action="', '"');
    $post_elements = $this->getHiddenElements($res);
    $post_elements['compose_from'] = "{$this->service_user}@hushmail.com";
    $post_elements['compose_encrypt'] = 'on';
    $post_elements['compose_sign'] = 'on';
    $post_elements['compose_save'] = 'on';
    $post_elements['action'] = 'Add contact (to)';
    $res = $this->post($form_action, $post_elements, TRUE);
    ...
    foreach ($contacts_string_array as $temp) {
      ...
      if (isset($contacts_array_temp[1])) $contacts[$contacts_array_temp[1]] = array('first_name' => (isset($contacts_array_temp[0]) {
        ? $contacts_array_temp[0] : FALSE), 'email_1' => $contacts_array_temp[1]);
      }
    }
