...
class facebook extends openinviter_base {
  ...
  public function getMyContacts() {
    ...
    while (preg_match_all("#\{\"id\"\:(.+)\,\"title\"\:\"(.+)\"#U", $res, $matches)) {
      ...
      if (!empty($matches[1])) foreach ($matches[1] as $key => $fbId) if (!empty($matches[2][$key])) {
        $contacts[$fbId] = $matches[2][$key];
      }
    }
    return $contacts;
  }
  ...
  public function logout() {
    ...
    if (!empty($res)) {
      $res = $this->get('http://www.facebook.com/logout.php?h='. html_entity_decode($this->getElementString($res, 'http://www.facebook.com/logout.php?h=', '"')));
    }
    $this->debugRequest();
    $this->resetDebugger();
    $this->stopPlugin();
