...
class tagged extends openinviter_base {
  ...
  public function getMyContacts() {
    ...
    if (preg_match_all("#\{\"name\"\:\"(.+)\"\,\"id\"\:(.+)\}#U", $res, $matches)) {
      if (!empty($matches[1])) foreach ($matches[1] as $key => $value) if (!empty($matches[2][$key])) {
        $contacts[$matches[2][$key]] = $value;
      }
    }

    reset($contacts);
    $firstKey = key($contacts);
    if (preg_match("#\[\{\"name\"\:\"(.+)\"\,\"id\"\:(.+)\}#U", $res, $matches)) {
      $contacts[$firstKey] = $matches[1];
    }
