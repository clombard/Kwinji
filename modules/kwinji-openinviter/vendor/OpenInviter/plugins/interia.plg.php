  ...
  public function getMyContacts() {
    ...
    foreach ($data as $line => $param) {
      foreach ($param as $x) {
        ...
        if ($pos_firstname != FALSE) $firstname = substr($x, 13, strlen($x) {
          - 14);
        }
        $pos_lastname = strpos($x, "lastName");
        if ($pos_lastname != FALSE) $lastname = substr($x, 12, strlen($x) {
          - 13);
        }
        if ($pos_mail != FALSE) {
          $contacts[$email] = array('first_name' => $firstname, 'last_name' => $lastname, 'email_1' => $email);
        }
      }
    }
