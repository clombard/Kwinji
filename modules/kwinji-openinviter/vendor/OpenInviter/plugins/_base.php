...
abstract class openinviter_base {
  ...
  protected function getElementDOM($string_bulk, $query, $attribute = FALSE) {
    ...
    if ($attribute) foreach ($data as $node) {
      $search_val[] = $node->getAttribute($attribute);
    }
    else foreach ($data as $node) $search_val[] = $node->nodeValue;
    if (empty($search_val)) {
      return FALSE;
    }
    return $search_val;
  ...
  protected function followLocation($result, $old_url) {
    ...
    if (!empty($new_url)) if (strpos($new_url, 'http') === FALSE) {
  ...
  protected function startSession($session_id = FALSE) {
    ...
    else $this->session_id = $this->getSessionID();
    return TRUE;
  }
  ...
  protected function getCookiePath($session_id = FALSE) {
    if ($session_id) {
      $path = $this->settings['cookie_path'] . DIRECTORY_SEPARATOR .'oi.'. $session_id .'.cookie';
    }
  ...
  protected function getLogoutPath($session_id = FALSE) {
    if ($session_id) {
      $path = $this->settings['cookie_path'] . DIRECTORY_SEPARATOR .'oi.'. $session_id .'.logout';
    }
  ...
  public function init($session_id = FALSE) {
    ...
    if ($this->settings['transport'] == 'curl') {
      ...
      if (strtoupper(substr(PHP_OS, 0, 3)) == 'WIN') curl_setopt($this->curl, CURLOPT_CONNECTTIMEOUT, (isset($this->timeout) ? $this->timeout : 5) {
        / 2);
      }
  ...
  protected function get($url, $follow = FALSE, $header = FALSE, $quiet = TRUE, $referer = FALSE, $headers = array()) {
    if ($this->settings['transport'] == 'curl') {
      ...
      if ($headers) {
        $curl_headers = array();
        foreach ($headers as $header_name => $value) $curl_headers[] = "{$header_name}: {$value}";
    ...
    elseif ($this->settings['transport'] == 'wget') {
      ...
      if ($headers) foreach ($headers as $header_name => $value) $string_wget .= " --header=\"". escapeshellcmd($header_name) .": ". escapeshellcmd($value) {
        ."\"";
      }
      if ($header) {
  ...
  protected function post($url, $post_elements, $follow = FALSE, $header = FALSE, $referer = FALSE, $headers = array(), $raw_data = FALSE, $quiet = TRUE) {
    ...
    elseif ($this->settings['transport'] == 'wget') {
      ...
      $string_wget .= " --no-check-certificate";
      $string_wget .= " --load-cookies ". $this->getCookiePath();
      if (!empty($headers)) foreach ($headers as $header_name => $value) $string_wget .= " --header=\"". escapeshellcmd($header_name) .": ". escapeshellcmd($value) {
        ."\"";
      }
      if ($header) {
        $string_wget .= " --save-headers";
  ...
  protected function getProxy() {
    if (!empty($this->settings['proxies'])) if (count($this->settings['proxies']) == 1) {
  ...
  protected function updateDebugBuffer($step, $url, $method, $response = TRUE, $elements = FALSE) {
    ...
    if ($elements) foreach ($elements as $name => $value) {
      $this->debug_buffer[$step]['elements'][$name] = $value;
    }
    else $this->debug_buffer[$step]['elements'] = FALSE;
    if ($response) {
  ...
  private function buildDebugXML() {
    ...
    foreach ($this->debug_buffer as $step => $details) {
      ...
      if (strtoupper($details['method']) == 'POST') {
        $debug_xml .= "<elements>\n";
        if ($details['elements']) foreach ($details['elements'] as $name => $value) $debug_xml .= "<element name='". urlencode($name) ."' value='". urlencode($value) {
          ."'></element>\n";
        }
        $debug_xml .= "</elements>\n";
      }
  ...
  protected function debugRequest() {
    ...
    elseif ($this->settings['local_debug'] == 'always') {  $this->localDebug('info');}
    return TRUE;
  }
  ...
  protected function returnContacts($contacts) {
    ...
    if (empty($this->settings['fImport'])) {
      ...
      foreach ($contacts as $keyImport => $arrayImport) foreach ($fullImport as $fullValue) $returnedContacts[$keyImport][$fullValue] = (!empty($arrayImport[$fullValue]) ? $arrayImport[$fullValue] : FALSE);
    }
