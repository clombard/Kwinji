...
class bordermail extends openinviter_base {
  ...
  public function getMyContacts() {
    ...
    if ($this->checkResponse("export_page", $res)) {
      ...
      foreach ($data as $val) if (strstr($val->nodeValue, "Import/Export")) $url_export = $val->getAttribute('href') {   ."&gab=1"; }
    ...
    if ($this->checkResponse('file_contacts', $res)) {
      ...
      foreach ($temp as $values) {
        ...
        if (!empty($values['4'])) $contacts[$values['4']] = (empty($name) {
          ? $values['4'] : $name);
        }
        if (!empty($values['12'])) $contacts[$values['12']] = (empty($name) {
          ? $values['12'] : $name);
        }
        if (!empty($values['13'])) $contacts[$values['13']] = (empty($name) {
          ? $values['13'] : $name);
        }
      }
      ...
      foreach ($temp as $tempA) {
        ...
        foreach ($arrayDescriptionFlag as $key => $value) $descriptionArray[(!empty($name) ? $name : (!empty($tempA[4]) ? $tempA[4] : FALSE))][$value] = isset($tempA[$key]) ? $tempA[$key] : FALSE;
      }
