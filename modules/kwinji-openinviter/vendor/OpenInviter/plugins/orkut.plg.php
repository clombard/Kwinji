...
class orkut extends openinviter_base {
    ...
    foreach ($originalLink as $link) {
      ...
      foreach ($data as $node) {
        ...
        if (isset($firstChild)) if ($firstChild->nodeName == 'a') if (strpos((string)$firstChild->getAttribute('href'), '/FullProfile?uid=') !== FALSE) {
          $id = str_replace('/FullProfile?uid=', '', (string)$firstChild->getAttribute('href'));
          $name = trim(preg_replace('/[^(\x20-\x7F)]*/', '', (string)$firstChild->nodeValue));
          if (!empty($id)) {
            $contacts[$id] = $name;
          }
        }
  ...
  public function sendMessage($session_id, $message, $contacts) {
    ...
    foreach ($contacts as $id => $name) {
      ...
      $post_elements['scrapText'] = str_replace(array('.', '&'), '~', $message['body']);
