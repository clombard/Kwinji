...
class rambler extends openinviter_base {
  ...
  public function getMyContacts() {
    ...
    if (!empty($emailsArray)) foreach ($emailsArray as $key => $emailValue) $contacts[trim($emailValue)] = array('first_name' => (isset($namesArray[$key]) ? trim($namesArray[$key]) {
      : FALSE), 'email_1' => trim($emailValue));
    }
