...
class _hosted extends openinviter_base {
  ...
  public function getHostedServices() {
    ...
    $services_cache = FALSE;
    if (file_exists($path)) if (time() - filemtime($path) <= 7200) {
      $services_cache = TRUE;
    }
    if (!$services_cache) {
      if (!$this->init()) {
      ...
      if (empty($res)) {
        $this->internalError = "Unable to connect to server.";
        return array();
      }
      if (strpos($res, "ERROR: ") === 0) {
        $this->internalError = substr($res, 7);
        return array();
      }
  ...
  public function login($user, $pass) {
    if (!isset($this->hostedServices['email'][$this->service])) {
      $this->internalError = "Unknown service.";
      return FALSE;
    }
    $this->resetDebugger();
    ...
    if (!$res) {
      $this->internalError = "Unable to import contacts. Please try again later.";
      return FALSE;
    }
    ...
    if ((string)$parsed_res->error != 'OK') {
      $this->internalError = (string)$parsed_res->error;
      return FALSE;
    }
    $this->contacts = array();
