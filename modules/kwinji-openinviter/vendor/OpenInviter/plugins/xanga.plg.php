...
class xanga extends openinviter_base {
  ...
  public function login($user, $pass) {
    ...
    $post_elements['txtSigninUsername'] = $user;
    $post_elements['txtSigninPassword'] = $pass;
    $post_elements['cmbNetwork'] = 1;
    $post_elements['signInButton'] = 'Sign In';
  ...
  public function getMyContacts() {
    ...
    $query = "//a[@class='thumbnail']";
    $data  = $xpath->query($query);
    foreach ($data as $node) if (strpos($node->getAttribute('title'), 'Visit') !== FALSE) {
      $href = $node->getAttribute('href');
      if (!empty($href)) {
        $name = $this->getElementString($href, '//', '.');
      }
      if (!empty($name)) $contacts[$href] = !empty($name) {
        ? $name : FALSE;
      }
    }
    return $contacts;
  ...
  public function sendMessage($session_id, $message, $contacts) {
    ...
    foreach ($contacts as $href => $name) {
      ...
      $post_elements = $this->getHiddenElements($res);
      $post_elements['messagesubject'] = $message['subject'];
      $post_elements['messagetext'] = $message['body'];
      $post_elements['btnSubmit'] = 'Submit';
      $res = $this->post($form_action, $post_elements, TRUE);
