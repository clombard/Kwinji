...
class inbox extends openinviter_base {
  ...
  public function login($user, $pass) {
    ...
    $url_contacts       = array();
    $url_contacts       = array($base_url, $url_contacts_array[0]);
  ...
  public function getMyContacts() {
    ...
    $form_action = $url[0] ."default.aspx";
    $post_elements = $this->getHiddenElements($res);
    $post_elements['cmdADDR'] = "To: Addr";
    ...
    $contacts = array();
    $page = 1;
    $total_nr_friends_array = explode("of ", $this->getElementString($res, '<td align="center">', '<'));
    if (!empty($total_nr_friends_array[1])) {
      $total_nr_friends = trim($total_nr_friends_array[1]);
    }
    else $total_nr_friends = 0;
    $friend_contor = 0;
    while ($total_nr_friends > $friend_contor) {
      ...
      foreach ($contacts_temp as $value) {
        ...
        if (!empty($contacts_array[1])) $contacts[str_replace(">", "", $contacts_array[1])] = array('first_name' => (!empty($contacts_array[0]) ? trim($contacts_array[0]) {
          : FALSE), 'email_1' => str_replace(">", "", $contacts_array[1]));
        }
        else $contacts[$value] = array('first_name' => FALSE, 'email_1' => $value);
        $friend_contor++;
      }
      $page++;
      $res = $this->get("{$form_action}?ACT=CPAB&AF=1&CID=-1&PG={$page}");
    }
