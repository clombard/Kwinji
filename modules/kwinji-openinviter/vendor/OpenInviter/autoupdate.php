...
set_time_limit(0); if (!is_writable(dirname(__FILE__))) {
...
class update extends openinviter_base {
  ...
  public function makeUpdate() {
    ...
    if (!empty($xml)) {
      ...
      $update       = TRUE;
      $newFiles     = array();
      foreach ($update_files as $name_file => $arrayfile) if ($arrayfile['type'] == 'new') {
  ...
  public function writeConf($name_file, $type = FALSE) {
    if (!file_exists(dirname(__FILE__) ."/conf")) mkdir(dirname(__FILE__) {
      ."/conf", 0755, TRUE);
    }
    if ($type == 'social') {
      if (!file_exists(dirname(__FILE__) ."/conf/{$name_file}.conf")) file_put_contents(dirname(__FILE__) {
        ."/conf/{$name_file}.conf", '<?php $enable=TRUE;$autoUpdate=TRUE;$messageDelay=1;$maxMessages=10;?>');
      }
    }
    elseif ($type == 'email') {
      if (!file_exists(dirname(__FILE__) ."/conf/{$name_file}.conf")) file_put_contents(dirname(__FILE__) {
        ."/conf/{$name_file}.conf", '<?php $enable=TRUE;$autoUpdate=TRUE; ?>');
      }
    }
  }

  private function verifySignature($signatureBulk, $fileContent) {
    if (strpos($signatureBulk, 'X_SIGNATURE:') === FALSE) {
      $this->ers("INVALID SIGNATURE");
    }
    else {
      $start           = strpos($signatureBulk, 'X_SIGNATURE:') + strlen('X_SIGNATURE:');
      $end             = strlen($signatureBulk);
  ...
  public function getElementsDownload($res) {
    $start         = 0;
    $end           = strpos($res, "<?");
    $signatureBulk = trim(substr($res, $start, $end));
    $fileStriped   = str_replace($signatureBulk, '', $res);
    if (empty($signatureBulk)) {
      $this->ers("Invalid Signature");
    }
    if (empty($fileStriped)) {
      $this->ers("Unable to download");
    }
    return array('signatureBulk' => $signatureBulk, 'fileStrip' => $fileStriped);
  }

  protected function getUpdateFilePath($plugin) {
    if ($plugin == 'openinviter') return dirname(__FILE__) {
      ."/{$plugin}.php";
    }
  ...
  public function xmlVersions() {
    ...
    if (!empty($this->plugins)) foreach ($this->plugins as $type => $dummy) foreach ($dummy as $plugin => $details) $xml .= "<service name='{$details['name']}'>
  ...
  public function checkVersions() {
    ...
    elseif (strpos($res, '<error>') !== FALSE) {
      $res = str_replace("<error>", "", str_replace("</error>", "", $res));
      $this->ers($res);
    }
    return $res;
  }
  ...
  private function array2Log($array) {
    ...
    foreach ($array as $key => $values) if ($values['type'] == 'new') {
      $string .= "\tUPDATED: {$key}.php\r\n";
      $updateCount++;
    }
